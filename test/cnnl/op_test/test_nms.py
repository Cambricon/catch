...
class TestNmsOp(TestCase):
    ...
    def test_nms1(self):
        ...
        boxes_mlu = boxes.to(ct.mlu_device())
        scores_mlu = scores.to(ct.mlu_device())
    ...
    def test_nms2(self):
        ...
        keep_indices = torchvision.ops.nms(
            boxes.to(ct.mlu_device()), scores.to(ct.mlu_device()), 0.5)
        keep_indices = np.sort(keep_indices.cpu())
    ...
    def test_nms_exception(self):
        ...
        ref_msg = r"^boxes and scores should have same number of elements in dimension 0, "
        ref_msg = ref_msg + r"got 2 and 3$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to(ct.mlu_device()), scores.to(ct.mlu_device()), 0.5)
        ...
        boxes = torch.from_numpy(
            np.array([10, 10, 50, 60, 11, 12, 48, 60]).astype(np.float32).reshape(-1, 4))
        scores = torch.from_numpy(np.array([0.5, 0.7]).astype(np.float32))
        ref_msg = r"^scores must be a MLU tensor$"
        with self.assertRaisesRegex(RuntimeError, ref_msg):
            torchvision.ops.nms(boxes.to(ct.mlu_device()), scores, 0.5)
