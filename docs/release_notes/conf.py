...
latex_documents = [
    (master_doc, u'Cambricon-PyTorch-Release-notes-CN.tex', u'寒武纪PyTorch版本说明书',
     author, 'manual'),
]
...
man_pages = [
    (master_doc, 'Cambricon-PyTorch-Release-notes-CN', u'寒武纪PyTorch版本说明书',
     [author], 1)
]
...
if  osp.exists(r'./_ext/customdirective.py'):
    '''
    下面增加搜索路径，如果已经添加了，可以删除，如果_ext所在目录不和conf.py在同一文件加，请修改成实际的相对目录。
    '''
    sys.path.append(osp.join(osp.dirname(__file__), '.'))
