...
release = version
curfnpre=u'Cambricon-PyTorch-User-Guide-CN-v'
...
latex_documents = [
    (master_doc, curfn, u'寒武纪PyTorch用户手册',
     author, 'manual'),
]
...
man_pages = [
    (master_doc, 'Cambricon-PyTorch-User-Guide-CN', u'Cambricon PyTorch User Guide',
     [author], 1)
]
...
class TocTreeFilt(TocTree):
    ...
    def __parsemultitags(self,tagstr)->bool:
    ...
    def __returnnewcontent(self,onlystr,contentstr,contentlst):
        ...
        tagsflag = self.__parsemultitags(tagstr)
        if tagsflag:
    ...
    def __GetOnlyContent(self,content):
        ...
        #得到only字符串前面的空格数
        onlyspacecount=self.__countspaces(onlystr)
        ...
        for i,j in enumerate(content[1:]):
            if j=="":
                continue
            spacecount=self.__countspaces(j)
